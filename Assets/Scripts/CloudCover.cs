...
public class CloudCover : MonoBehaviour
{
    ...
    [Tooltip("For scaleRange, x is the min vlaue, and y is the max value.")]
    public Vector2 scaleRange = new Vector2(1,4);

    // Start is called before the first frame update
    void Start(){
        Transform parentsTrans = this.transform;
        GameObject cloudGO;
        Transform cloudTrans;
        SpriteRenderer sRend;
        float scaleMult;
        for(int i = 0; i < numClouds; i++) {
            // Create a new GameObject (from scratch!) and gets its Transform
            ...
            cloudTrans.SetParent(parentsTrans, true);
